...
const modalTemplate = `
  <div class="modal">
    ...
    <img class="modal-image" src="" alt="" width="800" />
...
function openModal(index) {
  ...
  const imgElement = modal.element().querySelector('.modal-image');
...
galleryContainer.addEventListener('click', event => {
  ...
  const index = images.findIndex(image => image.original === imageSrc);

  openModal(index);
});

modal.element().addEventListener('click', event => {
  if (event.target.classList.contains('prev')) changeImage(-1);
  if (event.target.classList.contains('next')) changeImage(1);
});